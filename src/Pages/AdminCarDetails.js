...
function AdminCarDetails() {
  ...
  return (
    <LayoutAdmin>
      ...
      {!editMode ? (
        <div>
          <p><strong>Numer rej.:</strong> {car.id}</p>
          {car.carType && (
            <p><strong>Typ:</strong> {car.carType.brand}, {car.carType.model}, {car.carType.nrOfSeats} miejsc</p>
          )}
          <p><strong>Rok:</strong> {car.year}</p>
          <p><strong>Kolor:</strong> {car.color}</p>
          <p><strong>Cena / dzień:</strong> {car.price_per_day} zł</p>
          <p><strong>Status:</strong> {car.status}</p>

          <button onClick={startEditing}>Edytuj</button>
          <button onClick={handleDeleteCar} style={{ marginLeft: '1rem' }}>Usuń</button>
        </div>
      ) : (
      ...
      <Link to="/admin-cars" style={{ marginTop: '1rem', display: 'inline-block' }}>
        Powrót do listy
      </Link>
    </LayoutAdmin>
  );
